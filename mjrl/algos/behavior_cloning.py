...
class BC:
    ...
    def compute_transformations(self):
        # get transformations
        if self.expert_s == [] or self.expert_s is None:
            in_shift, in_scale, out_shift, out_scale = None, None, None, None
        else:
            observations = self.expert_s
            actions = self.expert_a
    ...
    def fit(self, data, suppress_fit_tqdm=False, **kwargs):
        ...
        if self.save_logs:
            loss_val = (
                self.loss(data, idx=range(num_samples))
                .to("cpu")
                .data.numpy()
                .ravel()[0]
            )
