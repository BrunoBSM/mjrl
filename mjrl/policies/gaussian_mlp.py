...
class MLP(torch.nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.observation_dim = (
            env_spec.observation_dim if env_spec is not None else observation_dim
        )  # number of states
        self.action_dim = (
            env_spec.action_dim if env_spec is not None else action_dim
        )  # number of actions
        # self.device = device
        self.seed = seed
        ...
        self.log_std = torch.nn.Parameter(
            torch.ones(self.action_dim) * init_log_std, requires_grad=True
        )
        self.trainable_params = list(self.parameters())
        # transform variables
        self.in_shift, self.in_scale = torch.zeros(self.observation_dim), torch.ones(
            self.observation_dim
        )
        self.out_shift, self.out_scale = torch.zeros(self.action_dim), torch.ones(
            self.action_dim
        )
    ...
    def to(self, device):
        super().to(device)
        self.min_log_std = self.min_log_std.to(device)
        self.in_shift, self.in_scale = self.in_shift.to(device), self.in_scale.to(
            device
        )
    ...
    def set_transformations(
        ...
        **kwargs,
        ...
        self.in_shift, self.in_scale = in_shift.to(self.device), in_scale.to(
            self.device
        )
        self.out_shift, self.out_scale = out_shift.to(self.device), out_scale.to(
            self.device
        )
    ...
    def mean_kl(self, observations, *args, **kwargs):
        ...
        return self.kl_divergence(
            new_mean, old_mean, new_log_std, old_log_std, *args, **kwargs
        )

    def kl_divergence(
        self, new_mean, old_mean, new_log_std, old_log_std, *args, **kwargs
    ):
